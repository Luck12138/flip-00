...
\block{Introduction}{
\begin{description}
  \item This is a time-series problem. Many information are given about daily sales data.
And the goal is to to predict total sales for every product and store in the next
month.The raw datasets contain six files, which attributes are shown below.
\end{description}
\vspace{.5cm}
% \begin{tabular}{ c | c }
%     \toprule
%     Name     &  Attribute         \\
%     \midrule
%     sales_train.csv &  $\{F_1\}$, $\{F_2F_4\}$   \\

%      Median based OAM &  $\{F_1\}$, $\{F_2F_4\}$   \\
%      \bottomrule
% \end{tabular}
...
%   \centering 
%   shops.csv
%   &  {shops\_name,shops\_id} \\
%   \centering 
%   item\_categories.csv
%   &  {item\_categories\_name,item\_categories\_id} \\
%   \centering
